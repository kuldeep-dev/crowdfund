              ...
              <table id="example2" class="table table-bordered table-hover">
                ...
                <tr>
                  ...
                  <?= $this->Form->postLink(__('Disable '), ['action' => 'desable', $user->id], ['confirm' => __('Are you sure you want to disable # {0}?', $user->id),'class' => 'btn btn-success btn-xs']) ?>  
                    ...
                    <?php if($user['camp_status'] == 1){ ?>    
                  <?= $this->Form->postLink(__('Campaign Disable '), ['action' => 'campdisable', $user->id], ['confirm' => __('Are you sure you want to disable all Campaign # {0}?', $user->id),'class' => 'btn btn-success btn-xs']) ?>  
                    <?php }else{ ?>
                    <?= $this->Form->postLink(__('Campaign Enable'), ['action' => 'campenable', $user->id], ['confirm' => __('Are you sure you want to enable all Campaign # {0}?', $user->id),'class' => 'btn btn-danger btn-xs']) ?>     
