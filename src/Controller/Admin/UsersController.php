...

{
    ...
    public function campdisable($id = null) {
        $user = $this->Users->get($id);
        $post['camp_status'] = 0;
        $user = $this->Users->patchEntity($user, $post);
       if ($this->Users->save($user)) {

        $ms = '<table width="200" border="1"><tr><th scope="row">Name</th><td>'.$user->name.'</td></tr><tr><th scope="row">Email</th><td>'.$user->email.'</td></tr></table>';

        $messgae = "Your Account is deactivated by admin.Please contact";

        $email = new Email('default'); 
        $email->from(['kuldeepjha@avainfotech.com' => 'Crowd Funding'])
            ->template('usercampstatus')
            ->emailFormat('html')
            ->viewVars(['data' => $user])
            ->viewVars(['msg' => $messgae])
            ->to($user->email)
   ...
   public function campenable($id = null) {
    $user = $this->Users->get($id);
    $post['camp_status'] = 1;
    $user = $this->Users->patchEntity($user, $post);
 if ($this->Users->save($user)) {
    ...
    $ms = '<table width="200" border="1"><tr><th scope="row">Name</th><td>'.$user->name.'</td></tr><tr><th scope="row">Email</th><td>'.$user->email.'</td></tr></table>';
        ...
        $messgae = "Your Account is activated by admin.";
